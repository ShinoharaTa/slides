...
URL に `?preview` を付けると**閲覧モード**になる。PDF ビューワーのように上に題名バー、各ページの下にページ番号が付き、
プレゼン・全画面・発表者ビューの操作は出ない（キーも効かない）。スライドを公開するときはこの URL を渡す。

```
decks/2026-10-example-conf/slide.html?preview
```

`index.html` の各カードに「共有用（閲覧モード）で開く」のリンクがある。
